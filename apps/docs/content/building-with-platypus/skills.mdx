...
- **Argument hint** _(optional)_ — up to **120 characters** describing what text
  should follow the command when a person invokes this Skill.
- **User-invocable only** — keep this Skill out of the model's catalogue while
  leaving it available for direct invocation by a person.
...
the model is never told its name, and cannot load it by name. The Skill stays
available for direct invocation by a person; the flag narrows the model's view
